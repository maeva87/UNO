...
router.post('/create', (req, res) => {
    ...
    db.run(
        ...
        function (err) {
            if (err) return res.status(500).json({ error: 'Erreur serveur' });
            db.run('INSERT INTO lobby_players (lobby_id, player_id) VALUES (?, ?)', [this.lastID, owner_id]);
...
router.post('/join', (req, res) => {
    ...
    db.get('SELECT * FROM lobbies WHERE code = ?', [code], (err, lobby) => {
        ...
        db.get('SELECT * FROM lobby_players WHERE lobby_id = ? AND player_id = ?', [lobby.id, player_id], (err, existing) => {
            if (existing) return res.status(400).json({ error: 'Déjà dans ce lobby' });

            db.get('SELECT COUNT(*) as count FROM lobby_players WHERE lobby_id = ?', [lobby.id], (err, result) => {
                if (result.count >= lobby.max_players) return res.status(400).json({ error: 'Lobby plein' });

                db.run('INSERT INTO lobby_players (lobby_id, player_id) VALUES (?, ?)', [lobby.id, player_id], () => {
...
router.post('/leave', (req, res) => {
    ...
    db.run('DELETE FROM lobby_players WHERE lobby_id = ? AND player_id = ?', [lobbyId, player_id], (err) => {
        if (err) return res.status(500).json({ error: 'Erreur serveur' });
        res.json({ message: 'Lobby quitté' });
    });
});
